...
## F-NN — <Flow name> (story: US-NNN)

<One sentence stating the user goal this flow serves. Must reference the US-NNN story
from stage 02 that this flow implements. One flow maps to one story only.>
...
1. Any `## F-NN` heading lacks `(story: US-NNN)`.
2. A `US-NNN` referenced does not exist in `02-spec/stories/` (cross-stage check, soft-warn only).
...
- **One flow, one story** — splitting a story across multiple flows hides which screen sequence the user actually walks. Stage 02 already decomposed work into stories; stage 03 inherits that decomposition.
